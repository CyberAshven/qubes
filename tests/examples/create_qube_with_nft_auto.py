#!/usr/bin/env python
"""
Automated End-to-End Qube Creation & NFT Minting Test

Non-interactive version that uses environment variables.

Required environment variables:
- PLATFORM_BCH_MINTING_KEY: Platform's WIF private key
- RECIPIENT_BCH_ADDRESS: Recipient's Bitcoin Cash address

Optional:
- QUBE_NAME: Name for the Qube (default: TestQube)
- CREATOR_EMAIL: Creator's email (default: test@example.com)
- BCH_NETWORK: mainnet or chipnet (default: mainnet)
"""
...
async def main():
    ...
    """
    print("\n" + "=" * 80)
    print("AUTOMATED END-TO-END QUBE CREATION & NFT MINTING TEST")
    print("=" * 80 + "\n")
    ...
    qube_name = os.getenv("QUBE_NAME", "TestQube")
    creator_email = os.getenv("CREATOR_EMAIL", "test@example.com")
    recipient_address = os.getenv("RECIPIENT_BCH_ADDRESS")
    network = os.getenv("BCH_NETWORK", "mainnet")

    # Validate required environment variables
    if not recipient_address:
        print("❌ ERROR: RECIPIENT_BCH_ADDRESS environment variable not set")
        print("\nSet it with:")
        print("export RECIPIENT_BCH_ADDRESS='bitcoincash:qp...'")
        return 1

    if not os.getenv("PLATFORM_BCH_MINTING_KEY"):
        print("❌ ERROR: PLATFORM_BCH_MINTING_KEY environment variable not set")
        print("\nSet it with:")
...
Your purpose is to assist users with their questions and tasks."""
    ...
    print(f"   Creator:        {creator_email}")
    print(f"   Birth Time:     {datetime.fromtimestamp(qube.genesis_block.birth_timestamp, tz=timezone.utc).isoformat()}")
    ...
    try:
        ...
        print(f"\n⚠️  NFT verification check failed: {e}")
        logger.warning("nft_verification_check_failed", error=str(e))
